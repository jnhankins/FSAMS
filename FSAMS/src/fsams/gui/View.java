...
import fsams.components.HumanAgent;
...
public class View {
    ...
    public void updateWorld(ComponentManager componentManager, double dTime) {
        for (FSAMSComponent1D c : componentManager.getComponents()) {
            if (c instanceof HumanAgent) {
                HumanAgent cHA = (HumanAgent)c;
                c.setX1(cHA.getX1() + cHA.getVX()*dTime);
                c.setY1(cHA.getY1() + cHA.getVY()*dTime);
            }
        }
    }
